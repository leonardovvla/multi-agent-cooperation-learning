...
class MAPolicy(object):
    ...
    def process_state_batch(self, states):      #Not called here
        '''
    ...
    def process_observation_batch(self, obs):       #Not called here
        '''
            Batch obs together.
            Args:
    ...
    def reshape_ma_observations(self, obs):     #Only called by process_obs_batch, which is not caleld here
    ...
    # ========================================================================================================= #
    # ======================================== AUX METHODS FOR SESS_RUN ======================================= #
    # ========================================================================================================= #

    def prepare_input(self, observation, state_in, taken_action=None):      #Only called by act, which is not caleld here
    ...
    def preprocess_output(self, act_output):
    ...
    # observation: observation obtained by stepping the environment
    # sess_act: TensorFlow session to be run
    # extra_feed_dict: optional dictionary with extra information (used only in the metrics and training iterations)
    # other_outputs: optional dictionary for requestion other outputs than the std ones (used only in the metrics and training iterations)
    # replace: whether other_outputs completely overrides the std outputs (replace=True) or simply add to it (replace=False)

    def sess_run(self, observation, sess_act, extra_feed_dict={}, other_outputs={}, replace=False):    
        ...
        if other_outputs:
            if replace:
                ...
                outputs.update({'state':self.state_out})
            else: 
        ...
        # Prepare inputs by adding the state of the LSTM (if there is one) and by adding taken actions for the training
        inputs = self.prepare_input(observation=obs, state_in=self.state, taken_action=extra_feed_dict)

        # Link Placeholders
        feed_dict={}
        for k,v in inputs.items():
            if isinstance(k,str):
                feed_dict.update({self.phs[k]: v})
            else:
                feed_dict.update({k:v})

        # Run Session
        outputs = sess_act.run(outputs, feed_dict)

        #Feed last state of the LSTM back in
        ...
        if other_outputs:
            if replace:
                ...
                info = {'vpred': self.preprocess_output(outputs['vpred']),
                        'ac_logp': self.preprocess_output(outputs['ac_logp']),
                        'state': outputs['state']}
                info.update({k:v for k,v in outputs.items() if k not in info.keys() and k is not 'ac'})
                return self.preprocess_output(outputs['ac']), info
        else:
            info = {'vpred': self.preprocess_output(outputs['vpred']),
                    'ac_logp': self.preprocess_output(outputs['ac_logp']),
                    'state': outputs['state']}
    ...
    def act(self, observation, extra_feed_dict={}):     #Not called here - only for OpenAI's environments
        outputs = {
            'ac': self.sampled_action,
            'ac_logp': self.sampled_action_logp,
            'vpred': self.scaled_value_tensor,
    ...
    def get_variables(self):        #Not called here
    ...
    def set_state(self, state):     #Not called here
