...
def make_env(n_substeps=15, horizon=80, deterministic_mode=False,
    ...
    #=========================== Adding walls  ==========================#
    #===========================  & Placing:   ==========================#
    #================== Boxes, Ramps, Hiders and Seekers ================#
    #====================================================================#
    ...
    #====================================================================#
    #============== Adding Manipulation, Friction & Gravity =============#
    #====================================================================#
    ...
    #====================================================================#
    #========= Setting up Action, Observation & Reward Spaces ===========#
    #====================================================================#
    ...
    #Set preparation phase, during which the reward is zero
    env = PreparationPhase(env, prep_fraction=prep_fraction)

    #Discretize Action Space into bins: 3 rows of 11 bins (-1 to 1) for each agent
    env = DiscretizeActionWrapper(env, 'action_movement') 

    #Sets mask for which boxes are seen by each agent
    if np.max(n_boxes) > 0:
    ...
    #FOOD: Sets mask for which sites (?) are seen by each agent
    if n_food:
    ...
    #Setting masks for Locking boxes
    ...
    #Setting masks for Locking ramps and for determining which ramps are seen by which agents
    if n_ramps > 0:
    ...
    #Setting masks for Grabbing objects
    ...
    #Check statistics (num boxes locked, etc) of prep_phase
    if prep_obs:
        env = TrackStatWrapper(env, np.max(n_boxes), n_ramps, n_food)

    #Attribute observation space to each agent: separate dimensions for each agent
