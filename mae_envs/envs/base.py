...
def make_env(n_substeps=5, horizon=250, deterministic_mode=False, n_agents=3,
             n_boxes=2, n_ramps=1):
    '''
    ...
    env = Base(n_agents=n_agents, n_substeps=n_substeps, horizon=horizon,   #Create Base env
               deterministic_mode=deterministic_mode)
    env.add_module(RandomWalls(grid_size=30, num_rooms=4, min_room_size=6, door_size=2))    #Add walls
    if n_boxes > 0:                              #Add Boxes
        env.add_module(Boxes(n_boxes=n_boxes))
    if n_ramps > 0:                              #Add Ramps
        env.add_module(Ramps(n_ramps=n_ramps))
    env.add_module(Agents(n_agents))             #Add agentes
    env.reset()
    keys_self = ['agent_qpos_qvel']              #Set keys
    keys_mask_self = ['mask_aa_obs']
    keys_external = ['agent_qpos_qvel']
    keys_mask_external = []
    env = SplitMultiAgentActions(env)                         #Attribute action space to each agent
    env = DiscretizeActionWrapper(env, 'action_movement')     #Discretize Action Space into bins: 3 rows of 11 bins (-1 to 1) for each agent

    # print(env.action_space.spaces['action_movement'].spaces[0])
    # print(env.discrete_to_continuous_act_map)
    
    env = AgentAgentObsMask2D(env)                            #Set vision cone for each agent
    env = SplitObservations(env, keys_self + keys_mask_self)  #Attribute observation space to each agent: separate dimensions for each agent
