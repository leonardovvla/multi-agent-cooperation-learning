...
#Uniform placement below a threshold in the x direction (farther from the right wall)
def thresh_uniform_placement(grid, obj_size, metadata, random_state):
    grid_size = len(grid)
    thresh = 11 #2 in length
    pos = np.array([random_state.randint(1, grid_size - obj_size[0] - thresh),
                    random_state.randint(1, grid_size - obj_size[1] - 1)])

    return pos

#Uniform placement below a threshold in the x direction (farther from the right wall) and close to another object
def thresh_close_to_other_object_placement(object_type, object_index, radius_key):
    def thresh_close_placement_fn(grid, obj_size, metadata, random_state):
        ...
        thresh = (3.0/11.0)*grid_size + 0.25 #2 in length
        ...
        pos=[grid_size - obj_size[0] - thresh, grid_size - obj_size[0] - thresh]
        while(pos[0] >= grid_size - obj_size[0] - thresh):
    ...
    return thresh_close_placement_fn
...
def corner_placement(grid, obj_size_in_cells, metadata, random_state):
    half_grid_size = int(len(grid) / 2)
    pos = np.array([int(obj_size_in_cells[0]/2),
                    int(obj_size_in_cells[0]/2)])
    return pos
